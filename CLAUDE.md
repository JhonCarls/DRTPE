...
- **Announcements (sede scoping)**: `Announcement` carries its own `sede` column (target audience, distinct from the author). Scopes `bySede`/`globalPrincipal`/`visibleForSede` enforce that a branch portal sees **its own + institutional (`sede = null`)** announcements but never another branch's. `sede = null` renders as "Sede Central" via the appended `sede_label` accessor. Listings order institutional first with `orderByRaw('CASE WHEN sede IS NULL THEN 0 ELSE 1 END')`.
