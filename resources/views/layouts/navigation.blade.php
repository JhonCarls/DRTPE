<nav x-data="{ openSidebar: false }" class="relative z-50">
    ...
    <aside :class="openSidebar ? 'translate-x-0' : '-translate-x-full'"
        ...
        <div class="p-4 border-t border-slate-800 bg-slate-950/30 shrink-0">
            ...
            {{-- AYUDA: guía de uso del sistema (se abre en otra pestaña para no perder el trabajo en curso) --}}
            <a href="{{ route('manual.general') }}" target="_blank" rel="noopener"
               class="flex items-center gap-3 px-4 py-3 mb-3 rounded-xl font-bold text-xs bg-amber-500/10 hover:bg-amber-500 border border-amber-500/30 hover:border-amber-500 text-amber-300 hover:text-white transition-all duration-200 group decoration-none">
                ...
                <span class="flex-1 leading-tight">Manual de uso
                    <span class="block text-[9px] font-bold uppercase tracking-widest text-amber-500/70 group-hover:text-amber-100 mt-0.5">¿Cómo se usa el sistema?</span>
                </span>
                <i class="fa-solid fa-arrow-up-right-from-square text-[10px] opacity-60"></i>
            </a>

            @if(Auth::user()->role === 'admin')
                {{-- El administrador también acompaña a los operadores de Juliaca y Taraco --}}
                <a href="{{ route('manual.sede') }}" target="_blank" rel="noopener"
                   class="flex items-center gap-2 px-4 pb-3 -mt-1 text-[10px] font-bold text-slate-500 hover:text-amber-400 transition-colors decoration-none">
                    <i class="fa-solid fa-building-flag text-[10px]"></i>
                    <span>Ver también la guía de las sedes</span>
                </a>
            @endif
